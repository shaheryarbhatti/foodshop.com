    ...
    $menuProducts = $categories->flatMap(function ($category) use ($fallbackImage) {
        return $category->products->map(function ($product) use ($category, $fallbackImage) {
            return [
                ...
                'image_url' => $product->image ? asset('public/' . $product->image) : null,
...
function openModal(productId){activeProduct=products.find((p)=>Number(p.id)===Number(productId))||null;activeQty=1;if(!activeProduct){return}modalTitle.textContent=`${activeProduct.serial_number} ${activeProduct.title}`;if(activeProduct.image_url){modalImage.src=activeProduct.image_url;modalImage.alt=activeProduct.title;modalImage.classList.remove('is-hidden')}else{modalImage.removeAttribute('src');modalImage.alt='';modalImage.classList.add('is-hidden')}modalDescription.innerHTML=activeProduct.description||'';addonFields.innerHTML='';remarks.value='';qtyDisplay.textContent='1';activeProduct.addons.forEach((addon)=>addonFields.appendChild(addonField(addon)));initAddonSelect2();renderModalPrice();modal.show()}
